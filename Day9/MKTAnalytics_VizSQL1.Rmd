...
```{r review count by rating ascending}
sqldf("
...
```{r review count by rating descending}
sqldf("
      SELECT rating, COUNT(review_id) AS NumReviews
      FROM reviews
      GROUP BY rating
...
```{r histogram of all products average ratings}
...
```{r histogram of products average ratings}
...
```{r bar chart that shows product count by category}
# same bar chart by count
...
```{r average prices of skincare products by highlights part 1}
...
```{r average prices of skincare products by highlights part 2, fig.width=10,fig.height=11}
# vertical bar chart
...
```{r average star ratings of skincare products by highlights, fig.width=10,fig.height=11}
# now, let's merge the highlight_table with the products table
# and calculate the average by each highlight
